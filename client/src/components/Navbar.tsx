...
export default function Navbar() {
  ...
  const currentThemeIcon = darkTheme ? themeIcon[0].light : themeIcon[0].dark;
    ...
    <>
      <nav className="navbar">
        ...
        <ul className={`link-nav ${showLinks ? "show" : ""}`}>
          ...
          >
            <SvgIcons
              path={currentThemeIcon.path}
              height={currentThemeIcon.height}
              width={currentThemeIcon.width}
            />
          </span>
        </ul>
      </nav>
